...
describe('MapSettingsService', () => {
  ...
  describe('carga inicial', () => {
    ...
    it('con datos guardados que faltan un POI nuevo, lo rellena con su valor por defecto en su posición', () => {
      const saved = { poi: [{ id: 'poi.park', label: 'x', icon: 'x', description: 'x', visible: false }] };
  ...
  describe('mapStyles', () => {
    it('en tema claro, no incluye la paleta oscura', () => {
      ...
      expect(service.mapStyles().some(s => s.elementType === 'geometry' && !s.featureType)).toBe(false);
    });
    ...
    it('en tema oscuro, antepone la paleta oscura', () => {
      ...
      expect(service.mapStyles().some(s => s.elementType === 'geometry' && !s.featureType)).toBe(true);
    });
    ...
    it('oculta la geometría/etiquetas de todos los POI como base, y reactiva solo los visibles', () => {
      ...
      expect(styles).toContainEqual({ featureType: 'poi', elementType: 'geometry', stylers: [{ visibility: 'off' }] });
      expect(styles).toContainEqual({ featureType: 'poi', elementType: 'labels', stylers: [{ visibility: 'off' }] });
      // poi.park está visible por defecto → se reactiva con elementType 'all'.
      expect(styles).toContainEqual({ featureType: 'poi.park', elementType: 'all', stylers: [{ visibility: 'on' }] });
