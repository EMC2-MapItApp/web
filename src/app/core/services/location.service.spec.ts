...
describe('LocationService', () => {
  ...
  const pub = (over: Partial<Publication> = {}): Publication => ({
    id: 'p1', authorId: 'a1', publicationType: 'event', placeId: null,
    locationTypeId: 'lt1', title: 'Evento', description: 'Desc', startDate: '2026-08-01T10:00:00Z',
    endDate: null, lat: 40.4, lng: -3.7, requiredLevel: 0, metadata: {}, active: true,
    occupiedSlots: 3, visibility: 'PUBLIC', hasAccess: true, accessRequestPending: false,
    ...over,
  });
  ...
  it('getAll pide activeOnly=true y convierte publicaciones a MapLocation', () => {
    ...
    expect(result).toEqual([
      expect.objectContaining({ id: 'p1', name: 'Evento', locationTypeId: 'lt1', lat: 40.4, lng: -3.7 }),
    ]);
  });
  ...
  it('descarta publicaciones sin lat/lng (privadas sin acceso, enmascaradas por el backend)', () => {
    ...
    httpMock.expectOne(`${environment.apiPublicationsUrl}?activeOnly=true`)
  ...
  it('getById devuelve undefined si no existe ninguna localización con ese id', () => {
    ...
    httpMock.expectOne(`${environment.apiPublicationsUrl}?activeOnly=true`).flush([pub({ id: 'p1' })]);
