...
description: >
  ...
  plantilla), Stylelint (color hardcodeado) y Prettier (`format:check`) sobre los archivos
  tocados como primer paso mecánico — los tres son gate real en CI, sin deuda base que
  descontar. No revisa lógica de negocio, corrección funcional, backend ni tests — eso es
...
accesibilidad básica de este checklist. La deuda base de plantilla (`D-006-W`, 66 errores
a 2026-08-16) se pagó por completo el 2026-09-03 — `npm run lint` da 0 en todo el repo, así
que **cualquier error en un fichero tocado por el diff es nuevo y se reporta**, sin
necesidad de cruzarlo con `git diff` línea a línea. Antes de revisar los `.html` tocados a
mano:

1. `npx eslint <archivo1.html> <archivo2.html> ...` sobre los `.html` realmente tocados —
   sigue sin ser necesario `npm run lint`/`ng lint` sobre todo el repo, es una cuestión de
   alcance de la revisión (solo el diff), no de descontar deuda ajena.
2. Repórtalo con `category: 'interfaz-intuitiva'` (o `mobile-first` si aplica más al
...
3. Cualquier error de regla que NO empiece por `@angular-eslint/template/` (p. ej.
...
`_variables.scss`, que definen esos valores, y `$shadow-*`/`$route-glow`/`$backdrop-overlay`
en `_variables.scss` como fuente única de sombras y efectos fijos repetidos — un
`box-shadow`/`filter` nuevo con un rgba() literal en vez de una de estas variables, o uno
genuinamente nuevo sin variable equivalente, es un finding). El resto de reglas de
`stylelint-config-standard-scss` están desactivadas — no cubren nada de este checklist,
salvo `selector-pseudo-element-no-unknown` (con excepción explícita para `::ng-deep`,
necesario para penetrar Material) y un par de reglas mecánicas de formato/duplicados que sí
están activas pese a no ser sobre color (`declaration-block-no-duplicate-properties`,
`declaration-block-no-redundant-longhand-properties`, `block-no-empty`,
`declaration-property-value-keyword-no-deprecated`) — repórtalas también si el diff las
dispara, con la misma `category: 'tema-variables'`. La deuda base de color (`D-006-W`, ~246
avisos a 2026-08-16) se pagó por completo el 2026-09-03 — `npm run lint:style` da 0 en todo
el repo, así que **cualquier aviso en un fichero tocado por el diff es nuevo y se reporta**.

1. `npx stylelint <archivo1.scss> <archivo2.scss> ...` sobre los `.scss` realmente
   tocados — sigue sin ser necesario `npm run lint:style` sobre todo el repo, es alcance de
   revisión, no deuda que descontar.
2. Repórtalo con `category: 'tema-variables'`, citando la regla en el `summary`. Esto solo
   cubre el primer punto de "Temas, variables y mixins globales" (color hardcodeado); el
   reuso de mixins concretos sigue siendo criterio manual — Stylelint no sabe qué hace cada
   mixin.

## Formato automatizado (Prettier)

`npm run format:check` es gate real en CI desde que el `npm run format` global se aplicó
el 2026-09-03 (mismo cierre de `D-006-W`) — cero deuda de formato, así que cualquier
archivo tocado que no pase `format:check` es un finding.

1. `npx prettier --check <archivo1> <archivo2> ...` sobre los `.html`/`.scss` realmente
   tocados (el `.ts` lo cubre `angular-conventions-reviewer`, no lo dupliques).
2. Repórtalo con `category: 'tema-variables'` si es un `.scss`, o `'interfaz-intuitiva'` si
   es un `.html` sin más categoría mejor — cita el archivo, no hace falta línea exacta (el
   diff de formato suele tocar el fichero entero).
...
`home-shell.html`, `group-form-page.html`, `confirm-dialog`, patrón `aria-label`) y
actualizado el 2026-09-03 al cerrarse `D-006-W`: `eslint.config.js`/`stylelint.config.js`/
`format:check` pasan de deuda base descontable (66 errores de plantilla, ~246 avisos de
color, 125 archivos sin formatear) a gate real sin excepciones — cualquier aviso en un
fichero tocado por el diff es ya responsabilidad del diff. Este checklist cita archivos,
mixins y componentes concretos a propósito — es lo que lo hace verificable en vez de
genérico. Si al revisar notas que una cita ya no corresponde con el código (mixin
renombrado/eliminado, patrón sustituido por otro), no lo ignores en silencio: repórtalo
