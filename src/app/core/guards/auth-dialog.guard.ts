 ...
 * token o sin usuario cargado, en vez de redirigir abre `AuthRequiredDialogComponent` sobre
...
export const authDialogGuard: CanActivateFn = async () => {
  ...
  const { AuthRequiredDialogComponent } = await import('@shared/auth-required-dialog/auth-required-dialog');
